...
namespace DiscreteModels.Third
{
    public class BranchAndBoundSolver
    {
        ...
        public Edge[] BranchAndBound(int[,] matrix)
        {
            ...
            if (!noValues)
            {
                matrix = RemoveMaxCoef(matrix, out r, out c, out maxCoef);

                costWithoutEdge = maxCoef + lowestCost;//Cost if we don't use that edge
                ...
                {
                    matrix[r, i] = -1;          //delete row
                }
                for (int i = 0; i < matrix.GetLength(1); i++)
                {
                    matrix[i, c] = -1;          //delete column
                }
                var newcost = CalculateSomeCostWithNewEdge(matrix);
                costWithEdge = lowestCost + newcost;    //Cost if we use that edge
            ...
            if (costWithoutEdge < costWithEdge)//are we use or don't use that edge?
            {
                var rollbackMatrix = matrices.First(x => x.pathCost < costWithEdge).matrix;//rollback to better state(cost)
            ...
            else
            {
                ...
                if (_edges.Count >= 2 && _edges.SkipLast(1).Last().Source == _edges.Last().Destination)
                    matrix[_edges.SkipLast(1).Last().Destination, _edges.Last().Source] = -1;
        ...
        private static int[,] RemoveMaxCoef(int[,] matrix, out int a, out int b, out int c)
        {
            ...
            for (int i = 0; i < matrix.GetLength(0); i++)
            {
                for (int j = 0; j < matrix.GetLength(1); j++)
                {
                    ...
                    if (matrix[i, j] == 0)
                    {
                        coef = CalculateCoeficient(matrix, i, j);
                        if (coef > maxCoef)
                        {
                            maxCoefForRow = i;
                            maxCoefForCol = j;
